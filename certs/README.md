...
El script es idempotente: si los archivos ya existen, no se regeneran. Para forzar la regeneración (por ejemplo, después de cambiar SANs):
...
| `privacyidea.key`, `privacyidea.crt` | Certificado del servidor de PrivacyIDEA, firmado por la CA. Mismas SANs que el de OpenLDAP. |
...
- **PrivacyIDEA** monta solo los dos archivos que necesita (`privacyidea.crt` y `privacyidea.key`) en `/etc/privacyidea/ssl/`.
...
Esta CA y los certs son artefactos de laboratorio. La llave privada `ca.key` queda en el disco del desarrollador sin protección de paso. En un entorno real:

- la CA viviría en una máquina aislada o un HSM,
- las llaves privadas estarían cifradas con paso,
